...
class _UtilityOverviewMonthlyBoxState extends State<UtilityOverviewMonthlyBox>
  ...
  Future<void> _load({bool silent = false}) async {
    if (_loadingNow || _disposed || !mounted || !_screenActive) {
      return;
    }
    ...
    try {
      ...
      final results = await Future.wait<Object>([
        api.getMonthlySummary(facId: facId, month: widget.month),
        api.getVoltageStatus(facId: facId),
      ]).timeout(_requestTimeout);

      if (!mounted || _disposed || !_screenActive) {
        return;
      }

      final parsedItems = List<EnergyMonthlySummary>.from(
        results[0] as List<EnergyMonthlySummary>,
      );
      ...
      final healthValues = <double>[
        ...parsedItems
            ...
            .where((value) => value != 0),
        ...parsedVoltages
            .expand((item) => [item.minVol, item.maxVol])
            .where((value) => value != 0),
      ];
      ...
      debugPrint('[MONTHLY BOX ERROR] $exception');
