...
var h=class{
    ...
    async _loadWasmEntrypoint(e,i,r,t){
        if(!this._scriptLoaded){
            ...
            this._ttPolicy!=null&&(
                c=this._ttPolicy.createScriptURL(c)
            );
...
async function T(s,e,i){
    ...
    t=new Promise((n,a)=>{
        r=setTimeout(()=>{
            a(
                new Error(
                    `${i} took more than ${e}ms to resolve. Moving on.`,
                    {
                        cause:T
                    }
                )
...
var g=class{
    ...
    loadServiceWorker(e){
        if(!e){
            console.debug(
                "Null serviceWorker configuration. Skipping."
            );
...
Read more: https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts`
        ...
        let{
            ...
            serviceWorkerUrl:r=l(
                `flutter_service_worker.js?v=${i}`
            ),
            timeoutMillis:t=4e3
        }=e,
        ...
        this._ttPolicy!=null&&(
            n=this._ttPolicy.createScriptURL(n)
        );

        let a=navigator.serviceWorker
            .register(n)
            .then(
                o=>this._getNewServiceWorker(
                    o,
                    i
                )
            )
...
var k=s=>{
    ...
    return(i,r)=>(
        (
            async()=>{
                let t=await e,
                n=await WebAssembly.instantiate(
                    t,
                    i
                );
...
var I=(s,e,i,r)=>(
    window.flutterCanvasKitLoaded=(
        async()=>{
            ...
            let t=
                i.hasChromiumBreakIterators&&
                i.hasImageCodecs;

            if(
                !t&&
                e.canvasKitVariant=="chromium"
            ){
            ...
            s.flutterTT.policy&&(
                o=s.flutterTT.policy.createScriptURL(o)
            );
...
var b=async(s,e,i,r)=>{
    ...
    s.flutterTT.policy&&(
        n=s.flutterTT.policy.createScriptURL(n)
    );
...
var S=class{
    ...
    async loadEntrypoint(e){
        ...
        return a.setTrustedTypesPolicy(
            t.policy
        ),
    ...
    async load({
        ...
        let a=m=>{
            switch(m){
                case"skwasm":
                    return p.hasChromiumBreakIterators&&
                        p.hasImageCodecs&&
                        p.supportsWasmGC;
        ...
        return v.setTrustedTypesPolicy(
            d.flutterTT.policy
        ),
        this.didCreateEngineInitializer=
...
//
// KHÔNG:
//   - load flutter.js lần nữa
//   - register service worker
//
// Loader phía trên đã tồn tại.
// ============================================================
